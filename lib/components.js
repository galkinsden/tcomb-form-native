...
class Struct extends Component {
  ...
  isValueNully() {
    return Object.keys(this.refs).every(ref => {
      ...
      return child && child.isValueNully ? child.isValueNully() : true;
    });
  }
...
class Form extends React.Component {
  ...
  pureValidate() {
    return this.inputRef.current.pureValidate();
  }
  ...
  getComponent(path) {
    ...
    return path.reduce((input, name) => input.refs[name], this.inputRef.current);
  }
